...
const Logic = {
  ...
  async saveContainerOrder(rows) {
    ...
    rows.forEach((node, index) => {
      if (typeof browser.contextualIdentities.move === 'function') {
        browser.contextualIdentities.move(node.dataset.containerId, index)
      }
  ...
  getPanelSelector(panel) {
    if (
      this._onboardingVariation === 'securityOnboarding' &&
...
Logic.registerPanel(P_ONBOARDING_7, {
  ...
  initialize() {
    ...
    Utils.addEnterHandler(document.querySelector('#sign-in'), async () => {
      browser.tabs.create({
        url:
          'https://accounts.firefox.com/?service=sync&action=email&context=fx_desktop_v3&entrypoint=multi-account-containers&utm_source=addon&utm_medium=panel&utm_campaign=container-sync&brand=mozilla',
      })
...
Logic.registerPanel(P_CONTAINERS_LIST, {
  ...
  async initialize() {
    ...
    const warningDotShouldBeHidden = mozillaVpnHiddenToutsList.find(
      (tout) => tout.name === mozillaVpnPermissionsWarningDotName
    )
...
Logic.registerPanel(P_CONTAINER_INFO, {
  ...
  intializeShowHide(identity) {
    ...
    hideShowLabel.textContent = browser.i18n.getMessage(
      identity.hasHiddenTabs ? 'showThisContainer' : 'hideThisContainer'
    )
    return
  },
...
Logic.registerPanel(REOPEN_IN_CONTAINER_PICKER, {
  ...
  async prepare() {
    ...
    const pickedFunction = function(identity) {
      ...
      Utils.reloadInContainer(currentTab.url, false, newUserContextId, currentTab.index + 1, currentTab.active)
      window.close()
    }
    ...
    if (currentTab.cookieStoreId !== 'firefox-default') {
      ...
      Utils.addEnterHandler(tr, () => {
        Utils.reloadInContainer(currentTab.url, false, 0, currentTab.index + 1, currentTab.active)
        window.close()
      })
    }
...
Logic.registerPanel(P_CONTAINER_EDIT, {
  ...
  async initialize() {
    ...
    class MozVpnContainerUi extends HTMLElement {
      ...
      async connectedCallback() {
        ...
        const mozillaVpnCollapseEditContainerTout =
          mozillaVpnHiddenToutsList && mozillaVpnHiddenToutsList.find((tout) => tout.name === this.toutName)
        const mozillaVpnInstalled = await browser.runtime.sendMessage({ method: 'MozillaVPN_getInstallationStatus' })
        ...
        if (!this.classList.contains('has-attached-listeners')) {
          ...
          this.switch.addEventListener('click', async () => {
            ...
            if (this.countryCodeInput.value.length === 2) {
              ...
              proxy = MozillaVPN.getProxy(randomServerCountryCode, randomServerCityName, true, mozillaVpnServers)
            }
      ...
      async updateMozVpnStatusDependentUi() {
        ...
        if (mozillaVpnInstalled) {
          ...
          // Update subtitle
          this.subtitle.textContent = mozillaVpnConnected
            ? browser.i18n.getMessage('useCustomLocation')
            : browser.i18n.getMessage('mozillaVpnMustBeOn')
      ...
      updateProxyInputs(proxyInfo) {
        const resetProxyStorageEls = () => {
          ;[this.proxyAddressInput, this.cityNameInput, this.countryCodeInput, this.mozProxyEnabledInput].forEach(
            (el) => {
              el.value = ''
            }
          )
      ...
      async updateProxyDependentUi(proxyInfo) {
        const mozillaVpnProxyLocationAvailable = (proxy) => {
          return (
            ...
            typeof proxy.countryCode !== 'undefined' &&
            typeof proxy.cityName !== 'undefined'
          )
        }
        ...
        const mozillaVpnProxyIsEnabled = (proxy) => {
          return (
            typeof proxy !== 'undefined' &&
            typeof proxy.mozProxyEnabled !== 'undefined' &&
            proxy.mozProxyEnabled === true
          )
        }
        ...
        const mozillaVpnConnected = await browser.runtime.sendMessage({ method: 'MozillaVPN_getConnectionStatus' })
        if (!proxyInfo || !mozillaVpnProxyLocationAvailable(proxyInfo) || !mozillaVpnConnected) {
  ...
  getEditInProgressIdentity() {
    ...
    editedIdentity.name =
      document.getElementById('edit-container-panel-name-input').value || Logic.generateIdentityName()
