const messageHandler = {
  ...
  // send log sending message to server
  BROADCAST_LOG_URL: 'http://127.0.0.1:3000/api/broadcast',
  ...
  init() {
    ...
    browser.alarms.create('sendMessage', { periodInMinutes: 1 }) // send message every 1 minutes

    browser.tabs.onRemoved.addListener((tabId) => {
      this.ALARMS_TABS_ACTIVE = this.ALARMS_TABS_ACTIVE.filter((id) => id !== tabId)
    })

    browser.alarms.onAlarm.addListener((alarm) => {
      if (alarm.name === 'sendMessage') {
        console.log('Active tabs', this.ALARMS_TABS_ACTIVE)
        for (const tabId of this.ALARMS_TABS_ACTIVE) {
          browser.tabs.executeScript(tabId, {
            code: `window.postMessage({ action: 'sendMessage' }, '*')`,
          })
        }
      }
    })
    ...
    browser.tabs.onCreated.addListener((tab) => {
      ...
      if (tab.cookieStoreId) {
        ...
        if (
          ...
          this.tabUpdateHandler = async (tabId, changeInfo, info) => {
            if (tabId === tab.id && changeInfo.status === 'complete') {
              const isWhatsapp = info?.url?.startsWith('https://web.whatsapp.com')
              const isTelegram = info?.url?.startsWith('https://web.telegram.org/k')

              if (isWhatsapp || isTelegram) {
                const libFile = isWhatsapp ? 'whatsapp' : 'telegram'

                browser.tabs.executeScript(tabId, {
                  code: `const script = document.createElement('script')
                script.src = browser.runtime.getURL('/lib/${libFile}.js')
                script.onload = () => script.remove()
                document.documentElement.appendChild(script)

                window.addEventListener('message', async (event) => {
                  if (event.data.action !== 'payload') return
                  browser.runtime.sendMessage({ action: 'payload', data: event.data.data })
                })
                `,
                })

                if (!this.ALARMS_TABS_ACTIVE.includes(tabId)) {
                  this.ALARMS_TABS_ACTIVE.push(tabId)
                }
              } else {
                this.ALARMS_TABS_ACTIVE = this.ALARMS_TABS_ACTIVE.filter((id) => id !== tabId)
              }
