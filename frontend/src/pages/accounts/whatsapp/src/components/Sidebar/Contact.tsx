...
const Contact = ({ contact }) => {
  ...
  const { '*': id } = useParams()
  const isActive = id === String(contact.id)

  return (
    <Link
      className={`sidebar-contact ${isActive && 'sidebar-active'}`}
      to={`${contact.id}`}
      onClick={() => setUserAsUnread(contact.id)}
    >
      <div className='sidebar-contact__avatar-wrapper'>
        <img src={contact.profile_picture} alt={contact.profile_picture} className='avatar' />
      </div>
      <div className='sidebar-contact__content'>
        <div className='sidebar-contact__top-content'>
          <h2 className='sidebar-contact__name'> {contact.name} </h2>
          <span className='sidebar-contact__time'>{formatTime(lastMessage.time)}</span>
        </div>
        <div className='sidebar-contact__bottom-content'>
          ...
          <div className='sidebar-contact__icons'>
            ...
            {!!contact.unread && <span className='sidebar-contact__unread'>{contact.unread}</span>}
